export const PROFILE_SELECT = "id, email, full_name, role, is_active, menu_access, is_manager, managed_department";
...
export async function fetchProfileByUserId(supabase, userId) {
  ...
  if (error?.message?.includes("menu_access") || error?.message?.includes("is_active") || error?.message?.includes("is_manager") || error?.message?.includes("managed_department")) {
    ...
    profile = fallbackResult.data ? { ...fallbackResult.data, is_active: true, menu_access: [], is_manager: false, managed_department: null } : null;
